...
Если $g'(\mu)=0$, но $g''(\mu)\ne 0$, классический метод дает дисперсию 0 ($\xrightarrow{\P} 0$). Домножаем ряд Тейлора на $n$ (а не $\sqrt{n}$):
$$ n(g(\overline{X}) - g(\mu)) \xrightarrow{d} \frac{\sigma^2 g''(\mu)}{2} \chi^2_1 $$
*(Предел — гамма-распределение, а не нормальное!)*

\textbf{Многомерный Дельта-метод:} $\vec{W}_i$ --- выборка, $\E[\vec{W}_1]=\vec{\mu}$, $\Sigma = \Cov(\vec{W}_1)$.
$\sqrt{n}(g(\vec{\overline{W}}) - g(\vec{\mu})) \xrightarrow{d} N(0, \nabla g(\vec{\mu})^T \Sigma \nabla g(\vec{\mu}))$.
\textit{Быстрый градиент для $g(y,z) = z/y^k$:} $\nabla g = \left(-\frac{k\mu_z}{\mu_y^{k+1}}, \frac{1}{\mu_y^k}\right)^T$.
...
\begin{enumerate}
    \item \textbf{Внутреннее $\E$ (Оптимизация Тонелли):} Сначала берем $\E$ по непрерывной $Y$! Фиксируем $X=x$. $\psi(x) = \E[g(x,Y)] = \int g(x,y)p_Y(y)dy$.
    \item \textbf{Атомы $X$:} Найти точки разрыва $x_k$. Вероятности скачков: $p_k = F_X(x_k) - F_X(x_k-0)$.
...
\begin{enumerate}
    ...
    \item \textbf{Замена:} Вводим $U=X, Y=h(X,Z)$. Вычисляем якобиан $J = \partial h(x,z) / \partial z$. Новая плотность: $p_{X,Z} = p_{X,Y}(x, h(x,z)) \cdot |J|$.
    \item \textbf{ДИНАМИЧЕСКИЕ ГРАНИЦЫ (Критично!):} Подставляем $y=h(x,z)$ в индикаторы исходной плотности $I_{\{y \in D_y\}}$. Разрешаем систему неравенств относительно $x$.
    \textit{Пример:} $x \in (0,1), xz \in (0,1) \implies 0 < x < \min(1, 1/z)$.
    \item \textbf{Маргинализация:} Интегрируем $p_{X,Z}$ по $x$. Внимание: из-за $\min$ интеграл $p_Z(z)$ распадается на две ветви (напр. $z \le 1$ и $z > 1$).
    \item \textbf{Усл. плотность и ожидание:} $p_{X|Z} = p_{X,Z}/p_Z$. Вычисляем интеграл $\E[g|Z=z] = \int g(\dots) p_{X|Z}(x|z)dx$.
\end{enumerate}

\textbf{Трюк измеримости (Вынесение множителя):}
Относительно условия $Z$, сама $Z$ — константа.
$\E[XY | Z=Y/X] = \E[X(XZ)|Z] = \E[X^2 Z|Z] = Z \cdot \E[X^2|Z]$.
Это радикально упрощает интеграл: ищем только $\E[X^2|Z]$!
...
При $X \sim U(-a,a)$, плотность четная. Усл. распределение дискретно: $\P(X=\sqrt{z}|z) = \P(X=-\sqrt{z}|z) = 1/2$.
$\E[X|X^2=z] = \sqrt{z}\cdot 1/2 - \sqrt{z}\cdot 1/2 = 0$.


\section*{Характеристические функции}
$\varphi(t) = \E[e^{it\xi}]$. Свойства: $\varphi(0)=1$, $|\varphi(t)| \le 1$, $\varphi_{aX+b}(t) = e^{itb}\varphi_X(at)$.
$\xi \perp\!\!\perp \eta \implies \varphi_{\xi+\eta} = \varphi_\xi \varphi_\eta$. Обратное неверно. Моменты: $\E[X^k] = \varphi^{(k)}(0) / i^k$.
...
\textbf{Симметрия модулей:} Для $X \sim N(0, \sigma^2)$ или Лапласа $\E[X^{\text{нечет}}]=0$. Ковариация: $\Cov(|X|, X^2) = \E[|X|^3] - \E|X|\E[X^2]$. \textbf{Не нужно раскрывать модуль!}
...
\begin{itemize}
    ...
    \item \textbf{Exp($\theta$):} $1/\theta, \ 1/\theta^2, \ 2/\theta^2$. (В задаче КР2 $Exp(\theta)$ плотность $p(x)=\theta e^{-\theta x}$, поэтому $\E=1/\theta$)
...
\begin{enumerate}
    \item \textbf{Размерность:} Если $[X]\!=\!\text{м}$, то $[\Var X]\!=\!\text{м}^2$. Для $g(X)\!=\!1/X^2$, дисперсия $\sigma_{as}^2 \sim [g]^2 \!=\! \text{м}^{-4}$. Если $\sigma_{as}^2<0$ --- ошибка в матрице Якоби!
